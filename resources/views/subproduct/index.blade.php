...
@section('title', 'Subproducts for: ' . $product->name)

@section('content')
<div class="d-flex justify-content-between align-items-center mb-3">
  <a href="{{ route('products.index') }}" class="btn btn-outline-secondary">← Back to Products</a>
  <a href="{{ route('subproducts.create', $product) }}" class="btn btn-primary">Add Subproduct</a>
</div>
    ...
    <tbody>
      @forelse($subproducts as $sp)
        <tr>
          ...
          <td>
            <a href="{{ route('subproducts.edit', [$product, $sp]) }}" class="btn btn-sm btn-outline-secondary">Edit</a>
            <form action="{{ route('subproducts.destroy', [$product, $sp]) }}" method="POST" class="d-inline">
              @csrf @method('DELETE')
